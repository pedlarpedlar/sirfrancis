...
$humanPageFilter = "pv.url NOT LIKE '%/admin-sf/%' AND pv.url NOT LIKE '%log_action.php%' AND pv.url NOT LIKE '%update_end_time.php%'";
...
$emailLinkedVisitors = ($hasPageViews && $hasSessions) ? cbAdminRows($conn, "SELECT pv.url, pv.referrer_url, pv.timestamp, s.user_id, s.session_id, s.ip_address, COALESCE(u.username, 'Guest') AS visitor_name, u.email
    ...
    LIMIT 10") : [];
$topViewedPages = ($hasPageViews && $hasSessions) ? cbAdminRows($conn, "SELECT SUBSTRING_INDEX(REPLACE(REPLACE(pv.url, 'https://www.fishgelatine.co.za/v2', ''), 'https://www.fishgelatine.co.za/v2', ''), '#', 1) AS page_url, COUNT(*) AS views, MAX(pv.timestamp) AS last_seen
    FROM page_views pv
    LEFT JOIN sessions s ON pv.session_id = s.id
    $sessionGeoJoin
    WHERE $humanPageFilter
