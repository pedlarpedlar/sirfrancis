...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['id'])) {
        ...
        if (isset($_SESSION['admin_id'])) {
            ...
            if ($adminStmt) {
                ...
                if (mysqli_stmt_num_rows($adminStmt) > 0) {
                    // Admin_id is legitimate, proceed with the order deletion

                    // Prepare the delete statement (order_items has an ON DELETE CASCADE key so this should work just fine.)
                    ...
                    if ($deleteStmt) {
                        ...
                        if ($result) {
                            // Deletion successful
                            header("Location: manage_orders.php?delete=success");
                            exit();
                        } else {
                            // Deletion failed
                            header("Location: manage_orders.php?delete=error1");
                            exit();
                        }
                        ...
                        header("Location: manage_orders.php?delete=error2");
                        exit();
                    }
                }
            }
        ...
        header("Location: manage_orders.php?delete=unauthorized");
        exit();
    } else {
        // 'id' parameter not set in the URL
        header("Location: manage_orders.php?delete=error3");
        exit();
    }
} else {
    // Invalid request method
    header("Location: manage_orders.php?delete=error4");
    exit();
}