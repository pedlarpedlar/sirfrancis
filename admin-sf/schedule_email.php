...
<style>
    ...
    .broadcast-panel h2, .broadcast-panel h3 { color: #28364B; }
    ...
    .status-pill { display: inline-block; background: #f5f2ea; border: 1px solid #e0d7cc; color: #28364B; padding: 6px 10px; font-size: 13px; font-weight: 700; }
...
<script>
    ...
    function updatePreview() {
        ...
        var couponHtml = coupon ? '<table width="100%" style="margin:22px 0;border:2px dashed #28364B;background:#fbfaf7;"><tr><td align="center" style="padding:18px;"><div style="font-size:12px;letter-spacing:1px;text-transform:uppercase;color:#6b6070;">Coupon code</div><div style="font-size:28px;font-weight:700;color:#28364B;margin-top:6px;">' + escapeHtml(coupon) + '</div></td></tr></table>' : '';
        var ctaHtml = ctaLabel && ctaUrl ? '<table style="margin-top:22px;"><tr><td style="background:#28364B;"><a href="' + escapeHtml(ctaUrl) + '" style="display:inline-block;padding:12px 18px;color:#ffffff;text-decoration:none;font-size:14px;font-weight:700;">' + escapeHtml(ctaLabel) + '</a></td></tr></table>' : '';

        $('#email-preview').html('<table width="100%" style="max-width:680px;background:#fff;border-collapse:collapse;margin:auto;"><tr><td style="background:#28364B;padding:22px 28px;color:#CEBD88;font-size:21px;font-weight:700;">' + escapeHtml(heading) + '</td></tr>' + heroHtml + '<tr><td style="padding:28px;color:#51475a;font-size:15px;line-height:1.7;">' + body + couponHtml + ctaHtml + '</td></tr><tr><td style="background:#f5f2ea;padding:18px;text-align:center;color:#28364B;font-size:12px;">' + escapeHtml(subject) + '</td></tr></table>');
    }
