...
<title>Categories - Sir Francis Admin</title>
...
.category-admin-hero { background: var(--sf-navy); color: #fff; border-radius: 8px; padding: 22px; margin-bottom: 18px; }
.category-admin-hero h1 { color: var(--sf-gold); margin-bottom: 6px; }
.category-admin-panel { background: #fff; border: 1px solid var(--sf-border); border-radius: 8px; padding: 18px; margin-bottom: 18px; }
...
.category-display-table th { color: #28364B; font-size: 13px; text-transform: uppercase; }
.category-display-table input[type="number"] { max-width: 90px; }
.category-muted { color: #75675d; font-size: 13px; }
.category-admin-panel h3 { color:#28364B; font-size:19px; margin:20px 0 8px; }
@media (max-width: 767px) {
    ...
    .category-display-table tr { border: 1px solid var(--sf-border); border-radius: 8px; margin-bottom: 12px; padding: 8px; }
...
?>
    ...
    <div class="container category-admin-shell text-md-start">
        ...
        <div class="category-admin-panel">
            ...
            <p class="category-muted">This controls how grouped pricelist sections appear, including child categories. Example: put marine collagen first, then fish gelatine, peptides, sea moss, private labelling and other sections. Sheet syncs can add new category paths; new ones appear at the bottom until you order them here.</p>
            <form method="post" action="<?= cbManageCategoryText($categoryPageSlug) ?>">
                <input type="hidden" name="category_action" value="save_display_categories">
                <?php $i = 0; foreach ($displayRows as $row): ?>
                    <input type="hidden" name="category_name[<?= $i ?>]" value="<?= cbManageCategoryText($row['name']) ?>">
