...
function candybirdSendSheetSyncAdminEmail($summary, $sourceLabel) {
    ...
    if (empty($recipient) && isset($conn) && $conn instanceof mysqli) {
        ...
        if ($settings && ($row = $settings->fetch_assoc())) {
            ...
            $website_company_name = $website_company_name ?: ($row['website_company_name'] ?? 'Sir Francis');
        }
    }

    if (empty($recipient) || empty($smtp_server) || empty($smtp_username5) || empty($smtp_password5)) {
        error_log('Sir Francis sheet sync email skipped: SMTP or support recipient missing.');
        return false;
    }
    ...
    $subject = 'Sir Francis sheet sync ' . strtolower($statusText) . ' - ' . date('Y-m-d H:i');
    ...
    try {
        ...
        $mail->addAddress($recipient, 'Sir Francis Support');
        ...
        $mail->Body = $body;
        $mail->AltBody = "Sir Francis sheet sync {$statusText}\n"
            . "Source: {$sourceLabel}\n"
        ...
        error_log('Sir Francis sheet sync admin email failed: ' . $e->getMessage());
        return false;
    }
}
...
function candybirdSendHighDiscountAlertEmail($alerts, $sourceLabel) {
    ...
    if (empty($recipient) || empty($smtp_server) || empty($smtp_username5) || empty($smtp_password5)) {
        error_log('Sir Francis high discount alert skipped: SMTP or recipient missing.');
        return false;
    }
    ...
    try {
        ...
        $mail->Port = (int) ($smtp_port ?? 587);
        $mail->setFrom($smtp_username5, $website_company_name ?: 'Sir Francis');
        $mail->addAddress($recipient, 'Sir Francis Admin');
        if (!empty($smtp_username1) && $smtp_username1 !== $recipient) {
            $mail->addReplyTo($smtp_username1, 'Sir Francis');
        }
        $mail->isHTML(true);
        $mail->CharSet = 'UTF-8';
        $mail->Subject = 'Sir Francis high discount alert - ' . count($alerts) . ' product(s)';
        $mail->Body = $body;
        $mail->AltBody = "Sir Francis high discount alert\n\n" . implode("\n", array_map(static function ($alert) {
            return '#' . $alert['id'] . ' ' . $alert['title'] . ' - ' . number_format((float) $alert['discount_percent'], 2) . '% discount';
        }, $alerts));
        $mail->send();
        return true;
    } catch (Exception $e) {
        error_log('Sir Francis high discount alert email failed: ' . $e->getMessage());
        return false;
    }
}
