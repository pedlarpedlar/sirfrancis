...
try {
    $mailResult = cbCandybirdSendMail(
        ...
        $customerName,
        'EFT payment details for Order ' . $orderNumber . ' | Sir Francis',
        $body
    );
    if (empty($mailResult['success'])) {
        $mailError = $mailResult['error'] ?? 'unknown email error';
        error_log('Sir Francis EFT email failed for order ' . $orderNumber . ': ' . $mailError);
        echo json_encode(['success' => false, 'message' => 'EFT email could not be sent: ' . $mailError]);
        exit;
    }

    echo json_encode(['success' => true, 'message' => 'EFT payment email sent to the customer via ' . ($mailResult['sender'] ?? 'Sir Francis mail') . '.']);
} catch (Throwable $e) {
    error_log('Sir Francis EFT email exception for order ' . $orderNumber . ': ' . $e->getMessage());
