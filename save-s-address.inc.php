...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $firstName = $_POST['s_first_name'];
    $lastName = $_POST['s_last_name'];
    $streetAddress1 = $_POST['s_street_address_1'];
    $streetAddress2 = $_POST['s_street_address_2'];
    $city = $_POST['s_city'];
    $province = $_POST['s_province'];
    $country = $_POST['s_country'];
    $postCode = $_POST['s_post_code'];
    $phoneNumber = $_POST['s_phone_number'];
    $emailAddress = $_POST['s_email_address'];
    ...
    if (empty($firstName) || empty($lastName) || empty($streetAddress1) || empty($city) || empty($province) || empty($country) || empty($postCode) || empty($phoneNumber) || empty($emailAddress)) {
        ...
        logAction('User Shipping Address Update Failed Attempt', $error, $userId, $guestIdentifier);
        ...
        if (mysqli_stmt_num_rows($stmt_check_address) > 0) {
            ...
            $sql = "INSERT INTO user_addresses (shipping_first_name, shipping_last_name, shipping_street_address_1, shipping_street_address_2, shipping_city, shipping_province, shipping_country, shipping_post_code, shipping_phone_number, shipping_email_address, guest_identifier, user_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        }
        ...
        if (mysqli_stmt_affected_rows($stmt) > 0) {
            ...
            logAction('User Changed Shipping Address Successfully', 'from Profile page', $userId, $guestIdentifier);
            ...
            $error = "Error updating address. Please try again. " . mysqli_stmt_error($stmt);
            $response = array(
                "success" => false,
                "message" => $error
            );
