...
function fetchCartParameters($conn, $userId, $guestIdentifier, $productId) {
    ...
    // Close the cart parameters statement
    mysqli_stmt_close($cartParamsStmt);

    // Fetch details of the current product
    $productDetailsSql = "SELECT p.title, SUM(p.price - p.discount_amount), i.image_url
                          FROM product p
                          LEFT JOIN images i ON p.id = i.product_id
                          WHERE p.id = ? AND enabled = 1";
    ...
    mysqli_stmt_bind_result($productDetailsStmt, $title, $price, $imageUrl);
    mysqli_stmt_fetch($productDetailsStmt);

    // Close the product details statement
    mysqli_stmt_close($productDetailsStmt);

    return ['subtotal' => number_format($subtotal, 2), 'item_quantity' => $item_quantity, 'product' => ['title' => $title, 'price' => $price, 'image_url' => $imageUrl]];
}
