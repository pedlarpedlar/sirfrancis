...
// Sorting options (already defined in your script)
...
$selectedSort = isset($_POST['sort']) && array_key_exists($_POST['sort'], $sortOptions) ? $_POST['sort'] : $defaultSort;

// Get selected filters from AJAX request
...
if (!empty($_POST['searchTerm'])) {
    ...
    $escapedSearchTerm = $conn->real_escape_string($_POST['searchTerm']);
...
WHERE p.enabled = 1 "; // Or use a 1=1 always true condition to start WHERE clause
...
if (!empty($filters['sizes'])) {
    $sql .= " AND p.weight IN ('" . implode("','", $filters['sizes']) . "')";
}

if (!empty($filters['properties'])) {
    foreach ($filters['properties'] as $index => $property) {
        $sql .= " AND p.features LIKE '%" . $conn->real_escape_string(trim($property)) . "%'";
    }
}

if (!empty($filters['tags'])) {
    // Check if "Sale" tag is selected
    if (in_array('Sale', $filters['tags'])) {
        $sql .= " AND p.discount_rate > 0";
    } else {
        // Otherwise, filter by selected tags
        $sql .= " AND p.label IN ('" . implode("','", $filters['tags']) . "')";
    }
}

// Filter by price range
if (isset($filters['price_min']) && isset($filters['price_max'])) {
    $sql .= " AND p.price BETWEEN " . (float)$filters['price_min'] . " AND " . (float)$filters['price_max'];
}

$categoryFilter = $_POST['category'];

// Add category filter if set (including all descendants)
if (!empty($categoryFilter) && $categoryFilter !== NULL ) {
...
if ($result->num_rows > 0) {
   // echo $result->num_rows;exit;
    ...
    $productHtml = '<p>No products found. Reset your filters or refresh the page to see all products</p>';
    $productHtmlListView = '<p>No products found. Reset your filters or refresh the page to see all products</p>';
}


header('Content-Type: application/json');
echo json_encode([
    'gridView' => $productHtml,
    'listView' => $productHtmlListView
]);
