...
$wishlistItems = getWishlistItems($userId, $guestIdentifier);
...
foreach ($wishlistItems as $item) {
    ...
    $wishlist_table .= '<tr>';
    $wishlist_table .= '<th class="text-center" scope="row">';
    $wishlist_table .= '<img src="' . $image_url . '" alt="img" />';
    $wishlist_table .= '</th>';
    $wishlist_table .= '<td>';
    $wishlist_table .= '<span class="whish-title"><a href="product?id='.$item['id'].'">' . $item['title'] . ' ' . $item['weight'] . '</a></span>';
    $wishlist_table .= '</td>';
    $wishlist_table .= '<td class="text-center">';
    $wishlist_table .= '<span class="badge badge-danger position-static">In Stock</span>';
    $wishlist_table .= '</td>';
    $wishlist_table .= '<td class="text-center">';
    $wishlist_table .= '<div class="product-count style">';
    $wishlist_table .= '<div class="count d-flex justify-content-center">';
    $wishlist_table .= '<input type="number" min="1" max="100" step="1" value="1" class="add-to-cart-quantity" />';
    $wishlist_table .= '<div class="button-group">';
    $wishlist_table .= '<button class="count-btn increment"><i class="fas fa-chevron-up"></i></button>';
    $wishlist_table .= '<button class="count-btn decrement"><i class="fas fa-chevron-down"></i></button>';
    $wishlist_table .= '</div>';
    $wishlist_table .= '</div>';
    $wishlist_table .= '</div>';
    $wishlist_table .= '</td>';
    ...
    $wishlist_table .= "<span class='whish-list-price'>";

    if ($item['discount_rate'] > 0) {
        $wishlist_table .= "<del class='del'>R".number_format($item['price'], 2)."</del>";
        $wishlist_table .= "<span class='onsale'>R".number_format($discounted_price, 2)."</span>";
    } else {
        $wishlist_table .= "R".number_format($item['price'], 2);
    }
    ...
    $wishlist_table .= '</td>';
    $wishlist_table .= '<td class="text-center">';
    $wishlist_table .= '<a href="#" class="removeFromWishlist" data-product-id="' . $item['id'] . '"><span class="trash" ><i class="fas fa-trash-alt"></i></span></a>';
    $wishlist_table .= '</td>';
    $wishlist_table .= '<td class="text-center">';
    $wishlist_table .= '<a href="#" class="btn btn-dark btn--lg add-to-cart" data-product-id="' . $item['id'] . '">add to cart</a>';
    $wishlist_table .= '</td>';
    $wishlist_table .= '</tr>';
}
...
if (!empty($wishlist_table)) {
    // Display the table if $wishlist_table is not empty
    echo '
    <!-- product tab start -->
              ...
              <div class="row">
                <div class="col-12">
                    <div class="section-title text-center">
                        <h2 class="title pb-3 mb-3">Your wishlist is empty!</h2>
