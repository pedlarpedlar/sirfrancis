...
if (isset($smtp_server, $smtp_username5, $smtp_password5, $smtp_type, $smtp_port, $smtp_username1)) {
    try {
        ...
        $mail->Subject = 'Password Changed Successfully | CandyBird';
        ...
        $emailBody = str_replace('{reset_link}', 'https://www.candybird.co.za/forgot-password', $emailBody);
        $mail->Body = $emailBody;
        $mail->isHTML(true);
        $mail->send();
    } catch (Exception $e) {
    ...
    try {
        ...
        $adminMail->Subject = 'User Successfully Changed Password | CandyBird';
        $adminBody = file_get_contents('emails/email_changed_password_admin.php');
        $adminBody = str_replace('{recipient_name}', 'Admin', $adminBody);
        $adminBody = str_replace('{reset_link}', 'https://www.candybird.co.za/admin-cb/', $adminBody);
