...
function cbResendOrderEmailItemsHtml($conn, $orderId) {
    ...
    while ($item = $result->fetch_assoc()) {
        ...
        $productUrl = $sheetProduct ? getSheetProductUrl($sheetProduct, true) : ('https://www.candybird.co.za/product?id=' . rawurlencode($clearanceId !== '' ? 'CLR:' . $clearanceId : (string) $productId));
        ...
        $html .= '<tr>
            ...
            <td style="padding:12px 10px;border-bottom:1px solid #e8e1d7;vertical-align:top;"><a href="' . htmlspecialchars($productUrl, ENT_QUOTES, 'UTF-8') . '" style="color:#5b1178;text-decoration:none;font-weight:700;">' . $titleHtml . '</a></td>
...
$customerResult = cbCandybirdSendMail(
    $customerEmail,
    $fullCustomerName,
    'CandyBird | Order Confirmation | #' . $orderPadded,
    $customerBody,
...
if (filter_var((string) $adminRecipient, FILTER_VALIDATE_EMAIL)) {
    $adminResult = cbCandybirdSendMail(
        $adminRecipient,
        'Admin',
        'CandyBird | Order Received | #' . $orderPadded,
        $adminBody,
        [
            'reply_to_email' => $customerEmail,
            'reply_to_name' => $fullCustomerName ?: 'CandyBird customer',
            'prefer_mail_transport' => true,
        ]
    );
}
...
error_log('CandyBird resend order email failed for order ' . $orderPadded . ': customer=' . ($customerResult['error'] ?? 'ok') . ' admin=' . ($adminResult['error'] ?? 'ok'));
echo json_encode([
    'success' => false,
