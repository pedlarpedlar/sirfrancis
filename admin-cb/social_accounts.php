...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'save_settings') {
            ...
            $subject = cbOpsCleanHeader($_POST['subject'] ?? 'CandyBird social posting reminder');
            $enabled = !empty($_POST['enabled']) ? 1 : 0;

            $conn->query("INSERT IGNORE INTO admin_social_reminder_settings (id, recipient_email, reminder_day, reminder_time, subject, enabled, updated_at) VALUES (1, '', 'Monday', '08:00:00', 'CandyBird social posting reminder', 1, NOW())");
...
<title>Social Accounts & Reminders - CandyBird Admin</title>
    ...
    <div class="social-admin-side">
      <div class="ops-panel mb-4">
        ...
        <form method="post">
          ...
          <div class="form-group">
            <label>Subject</label>
            <input type="text" class="form-control" name="subject" value="<?= cbOpsText($settings['subject'] ?? 'CandyBird social posting reminder') ?>">
          </div>
      ...
      <div class="ops-panel" id="social-account-form">
        ...
        <form method="post">
          ...
          <div class="form-group">
            <label>Handle</label>
            <input type="text" class="form-control" name="handle" value="<?= cbOpsText($editing['handle'] ?? '') ?>" placeholder="@candybird">
          </div>
          <div class="form-group">
