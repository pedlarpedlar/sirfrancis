...
$intro = '<p class="mb-2">Manage coupon codes via a live Google Sheet. Download the coupon template, fill in your coupon rows from line 3, publish the current sheet to the web in TSV format, and save both links below.</p>'
    ...
    . '<p class="mb-2">Only one coupon can be used per order. Keep coupon dates, minimum order values, restrictions, and usage counts clean so checkout can calculate discounts correctly.</p>'
    . '<p class="mb-0"><a class="btn btn-outline-primary btn-sm" href="coupon_tester">Test a coupon before customers use it</a></p>';
