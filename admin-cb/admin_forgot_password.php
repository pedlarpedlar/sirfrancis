...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($username === '') {
        ...
        if (!$admin) {
            ...
            error_log('CandyBird admin password reset requested for admin without recovery email: ' . $admin['username']);
            $message = $neutralMessage;
        } else {
            try {
                ...
                error_log('CandyBird admin password reset email failed: ' . $e->getMessage());
                $message = $neutralMessage;
            }
        }
    }
